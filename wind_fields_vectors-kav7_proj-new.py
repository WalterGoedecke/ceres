...
sublats = [x for x in dirlats[::10]]
sublons = [x for x in dirlons[::10]]

dirus = winds.variables['U'][0, 0, :, :]
dirvs = winds.variables['V'][0, 0, :, :]

#subus = [x for x in dirus[::20, ::20]]

#a = [[i + j for j in range(3)] for i in range(3)]
#subus = [[y for y in dirus[::20, :] for x in dirus[::20, y]]
subus = [[dirus[i, j] for j in range(0, 192, 10)] for i in range(0, 94, 10)]
subvs = [[dirvs[i, j] for j in range(0, 192, 10)] for i in range(0, 94, 10)]
...
#Q = m.quiver(x, y, dirus, dirvs, scale=2000)
#Q = m.quiver(x, y, dirus, dirvs, scale=70)
Q = m.quiver(x, y, subus, subvs, scale=1200, cmap=plt.cm.jet)
...
# add a title.
ax.set_title('Wind Fields - Arrows')
plt.show()
