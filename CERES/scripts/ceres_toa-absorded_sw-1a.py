# -*- coding: utf-8 -*-
"""
Created on Fri Oct 23 15:28:51 2015
...
# # # # # # # # # # # #
toa_data = Dataset('../datasets/CERES_EBAF-TOA_Ed2.8_Subset_201401-201412-net_&_total_solar.nc')
...
print('times: %i\t lats: %i\t lons: %i\t net_all: %i\t solar: %i\n' % \
    (timesSize, latsSize, lonsSize, net_all.size, solar.size))
...
absorbed_avg_lats_jan = [y-x for x, y in zip(net_all_avg_lats_jan, solar_avg_lats_jan)]
...
absorbed_avg_lats_jul = [y-x for x, y in zip(net_all_avg_lats_jul, solar_avg_lats_jul)]
...
absorbed_avg_lats = [y-x for x, y in zip(net_all_avg_lats, solar_avg_lats)]
...
axis([-90, 90, 0, 1.2*amax(absorbed_avg_lats_jan) ])
plt.xticks(range(-90, 91, 30))
xlabel('Latitude (deg)')
ylabel('Radiative flux $(W/m^2)$')
