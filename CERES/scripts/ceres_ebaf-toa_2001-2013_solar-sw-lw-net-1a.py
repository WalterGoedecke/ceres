# -*- coding: utf-8 -*-
"""
Created on Fri Oct 30 13:38:26 2015
...
toa_data = Dataset('../datasets/CERES_EBAF-TOA_Ed2.8_Subset_200101-201312_solar-sw-lw-net.nc')
...
solar_avg_lats_jan = [ average([[solar[time, i, j] for j in range(0, lonsSize)] \
    for time in range(time, timesSize, 12)]) for i in range(0, latsSize) ]
...
# Calculate the February data.
time = 1
solar_avg_lats_feb = [ average([[solar[time, i, j] for j in range(0, lonsSize)] \
    for time in range(time, timesSize, 12)]) for i in range(0, latsSize) ]
sw_all_avg_lats_feb = [ average([[sw_all[time, i, j] for j in range(0, lonsSize)] \
    for time in range(time, timesSize, 12)]) for i in range(0, latsSize) ]
lw_all_avg_lats_feb = [ average([[lw_all[time, i, j] for j in range(0, lonsSize)] \
    for time in range(time, timesSize, 12)]) for i in range(0, latsSize) ]
net_all_avg_lats_feb = [ average([[net_all[time, i, j] for j in range(0, lonsSize)] \
    for time in range(time, timesSize, 12)]) for i in range(0, latsSize) ]
# # # # # # # # # # # #

# Calculate the December data.
time = 11
solar_avg_lats_dec = [ average([[solar[time, i, j] for j in range(0, lonsSize)] \
    for time in range(time, timesSize, 12)]) for i in range(0, latsSize) ]
sw_all_avg_lats_dec = [ average([[sw_all[time, i, j] for j in range(0, lonsSize)] \
    for time in range(time, timesSize, 12)]) for i in range(0, latsSize) ]
lw_all_avg_lats_dec = [ average([[lw_all[time, i, j] for j in range(0, lonsSize)] \
    for time in range(time, timesSize, 12)]) for i in range(0, latsSize) ]
net_all_avg_lats_dec = [ average([[net_all[time, i, j] for j in range(0, lonsSize)] \
    for time in range(time, timesSize, 12)]) for i in range(0, latsSize) ]
# # # # # # # # # # # #

# Calculate the January, February, & December average data.
solar_avg_lats_jan_feb_dec = [ average([x, y, z]) for x, y, z in zip(solar_avg_lats_jan, solar_avg_lats_feb, solar_avg_lats_dec) ]
sw_all_avg_lats_jan_feb_dec = [ average([x, y, z]) for x, y, z in zip(sw_all_avg_lats_jan, sw_all_avg_lats_feb, sw_all_avg_lats_dec) ]
lw_all_avg_lats_jan_feb_dec = [ average([x, y, z]) for x, y, z in zip(lw_all_avg_lats_jan, lw_all_avg_lats_feb, lw_all_avg_lats_dec) ]
net_all_avg_lats_jan_feb_dec = [ average([x, y, z]) for x, y, z in zip(net_all_avg_lats_jan, net_all_avg_lats_feb, net_all_avg_lats_dec) ]
# # # # # # # # # # # #

# Calculate the June data.
time = 5
solar_avg_lats_jun = [ average([[solar[time, i, j] for j in range(0, lonsSize)] \
    for time in range(time, timesSize, 12)]) for i in range(0, latsSize) ]
sw_all_avg_lats_jun = [ average([[sw_all[time, i, j] for j in range(0, lonsSize)] \
    for time in range(time, timesSize, 12)]) for i in range(0, latsSize) ]
lw_all_avg_lats_jun = [ average([[lw_all[time, i, j] for j in range(0, lonsSize)] \
    for time in range(time, timesSize, 12)]) for i in range(0, latsSize) ]
net_all_avg_lats_jun = [ average([[net_all[time, i, j] for j in range(0, lonsSize)] \
    for time in range(time, timesSize, 12)]) for i in range(0, latsSize) ]
# # # # # # # # # # # #
...
# Calculate the August data.
time = 7
solar_avg_lats_aug = [ average([[solar[time, i, j] for j in range(0, lonsSize)] \
    for time in range(time, timesSize, 12)]) for i in range(0, latsSize) ]
sw_all_avg_lats_aug = [ average([[sw_all[time, i, j] for j in range(0, lonsSize)] \
    for time in range(time, timesSize, 12)]) for i in range(0, latsSize) ]
lw_all_avg_lats_aug = [ average([[lw_all[time, i, j] for j in range(0, lonsSize)] \
    for time in range(time, timesSize, 12)]) for i in range(0, latsSize) ]
net_all_avg_lats_aug = [ average([[net_all[time, i, j] for j in range(0, lonsSize)] \
    for time in range(time, timesSize, 12)]) for i in range(0, latsSize) ]
# # # # # # # # # # # #
    
# Calculate the June, July, & August average data.
solar_avg_lats_jun_jul_aug = [ average([x, y, z]) for x, y, z in zip(solar_avg_lats_jun, solar_avg_lats_jul, solar_avg_lats_aug) ]
sw_all_avg_lats_jun_jul_aug = [ average([x, y, z]) for x, y, z in zip(sw_all_avg_lats_jun, sw_all_avg_lats_jul, sw_all_avg_lats_aug) ]
lw_all_avg_lats_jun_jul_aug = [ average([x, y, z]) for x, y, z in zip(lw_all_avg_lats_jun, lw_all_avg_lats_jul, lw_all_avg_lats_aug) ]
net_all_avg_lats_jun_jul_aug = [ average([x, y, z]) for x, y, z in zip(net_all_avg_lats_jun, net_all_avg_lats_jul, net_all_avg_lats_aug) ]
# # # # # # # # # # # #
...
title('Solar Incoming Short Wave Flux\nJanuary, July, and all Months in 2001-2013', fontsize=18)
grid(True)
savefig("../images/CERES_TOA_solar_incoming_avg_lats-jan_jul_2001-2013.png")
show()
...
xlabel('Latitude (deg)')
ylabel('Radiative flux $(W/m^2)$')
title('Solar Incoming Short Wave Flux\nWinter, Summer, and all Months in 2001-2013', fontsize=18)
grid(True)
savefig("../images/CERES_TOA_solar_incoming_avg_lats-jan-feb-dec_jun-jul-aug_2001-2013.png")
show()
...
grid(True)
savefig("../images/CERES_TOA_outgoing_sw_all_avg_lats-jan_jul_2001-2013.png")
show()
...
title('TOA Solar Short Wave Outgoing Flux, All Sky\n Winter, Summer, and all Months in 2001-2013', fontsize=18)
grid(True)
savefig("../images/CERES_TOA_outgoing_sw_all_avg_lats-jan-feb-dec_jun-jul-aug_2001-2013.png")
show()
#######################
...
grid(True)
savefig("../images/CERES_TOA_absorbed_sw_avg_lats-jan_jul_2001-2013.png")
show()
...
title('CERES TOA Absorbed Short Wave Flux, All Sky\n Winter, Summer, and all Months in 2001-2013', fontsize=18)
grid(True)
savefig("../images/CERES_TOA_absorbed_sw_avg_lats-jan-feb-dec_jun-jul-aug_2001-2013.png")
show()
#######################
...
grid(True)
savefig("../images/CERES_TOA_albedo_avg_lats-jan_jul_2001-2013.png")
show()
...
title('CERES Top of Atmosphere Albedo\n Winter, Summer, and all Months in 2001-2013', fontsize=18)
grid(True)
savefig("../images/CERES_TOA_albedo_avg_lats-jan-feb-dec_jun-jul-aug_2001-2013.png")
show()
...
grid(True)
savefig("../images/CERES_TOA_outgoing_lw_all_avg_lats-jan_jul_2001-2013.png")
show()
...
title('TOA Solar Long Wave Outgoing Flux, All Sky\n Winter, Summer, and all Months in 2001-2013', fontsize=18)
grid(True)
savefig("../images/CERES_TOA_outgoing_lw_all_avg_lats-jan-feb-dec_jun-jul-aug_2001-2013.png")
show()
...
xlabel('Latitude (deg)')
ylabel('Radiative flux $(W/m^2)$')
title('TOA Solar Net Incoming Short & Long Wave Flux, All Sky\n January, July, and all Months in 2001-2013', fontsize=18)
grid(True)
savefig("../images/CERES_TOA_incoming_net_all_avg_lats-jan_jul_2001-2013.png")
show()
...
title('TOA Solar Net Incoming Short & Long Wave Flux, All Sky\n Winter, Summer, and all Months in 2001-2013', fontsize=18)
grid(True)
savefig("../images/CERES_TOA_incoming_net_all_avg_lats-jan-feb-dec_jun-jul-aug_2001-2013.png")
show()
#######################
